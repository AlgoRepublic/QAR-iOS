...
//    dispatch_async(dispatch_get_main_queue(), ^(void) {
        ...
        setFakeRealTrackingObject(tracker7, tracker8);
//    });
        ...
        if (status == easyar_TargetStatus_Tracked) {
            ...
            else
            {
                ...
                if (![fakeTrackerArray containsObject: [NSNumber numberWithInt:[target runtimeID]]]){
                    ...
                    if (fakeTrackerArray.count == 4 && [detectedNote isEqualToString: [text componentsSeparatedByString:@" "].firstObject]){
                        NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:text,@"side",@(4),@"count", nil];
                        detectedNote = [text componentsSeparatedByString:@" "].firstObject;
//                        [AppUtils postRealNoteDetectedNotification:dic];
                    ...
                    } else if (fakeTrackerArray.count >= 8 && [detectedNote isEqualToString: [text componentsSeparatedByString:@" "].firstObject]){
                        
                        NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:text,@"side",@(8),@"count", nil];
                        
                        [fakeTrackerArray removeAllObjects]; //hassan commented and took one step up this line for after one note detection complition
...
//    start();
    ...
    [fakeTrackerArray removeAllObjects];
    [trackers removeAllObjects];
    [trackers addObject:tracker7];
    [trackers addObject:tracker8];
    start();
}
