...
-(void)realNoteDetected:(NSNotification*)notification{
    ...
    if ([noteSide isEqualToString:@"Back"] && [[dic valueForKey:@"count"] integerValue] == 4 ){
    ...
    }else if([noteSide isEqualToString:@"Front"] && [[dic valueForKey:@"count"] integerValue] == 4 ){
        if([AppUtils getLanguage] == MRAppLanguageArabic) {
            [FTIndicator showToastMessage:[NSString stringWithFormat:@"إقلب العملة"]];
            [self playSettingsAudio:@"Turn over Arabic"];
        }
    ...
    }else if([[dic valueForKey:@"count"] integerValue] == 8){
...
- (IBAction)aboutButtonTarget:(id)sender
{
    ...
    aboutView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [[UIScreen mainScreen]bounds].size.width, [[UIScreen mainScreen]bounds].size.height)];
